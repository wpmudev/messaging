...
$messaging_current_version = '1.1.6.7';
//------------------------------------------------------------------------//
//---Config---------------------------------------------------------------//
//------------------------------------------------------------------------//
...
function messaging_network_settings() {
			...
			case "process":
				...
				if ( is_multisite() ) {
					...
					<script language='JavaScript'>
						window.location = 'settings.php?page=messaging_settings&updated=true&updatedmsg=<?php echo urlencode( __( 'Settings saved.', 'messaging' ) ); ?>';
					</script>
					<?php
				} else {
					?>
					<script language='JavaScript'>
						window.location = 'admin.php?page=messaging_settings&updated=true&updatedmsg=<?php echo urlencode( __( 'Settings saved.', 'messaging' ) ); ?>';
					</script>
					<?php
				}
				break;
	...
	function messaging_insert_message( $tmp_to_uid, $tmp_to_all_uids, $tmp_from_uid, $tmp_subject, $tmp_content,
									$tmp_status, $tmp_official = 0 ) {
		global $wpdb;
		/*
		 ...
		 */
		$wpdb->insert( $wpdb->base_prefix . "messages", array(
	...
	function messaging_update_message_status( $tmp_mid, $tmp_status ) {
		...
		$wpdb->update( $wpdb->base_prefix . "messages", array( 'message_status' => $tmp_status ), array( 'message_ID' => $tmp_mid ), array( '%s' ), array( '%d' )
		);
	}
	...
	function messaging_insert_sent_message( $tmp_to_all_uids, $tmp_from_uid, $tmp_subject, $tmp_content, $tmp_official = 0 ) {
		 ...
		 */
		$wpdb->insert( $wpdb->base_prefix . "sent_messages", array(
		...
		switch ( $action ) {
			...
			default:
				if ( isset( $_POST[ 'Remove' ] ) ) {
					...
					?>
					<script language='JavaScript'>
						window.location = 'admin.php?page=messaging&updated=true&updatedmsg=<?php echo urlencode( __( 'Message removed.', 'messaging' ) ); ?>';
					</script>
					<?php
				}
				if ( isset( $_POST[ 'Reply' ] ) ) {
					?>
					<script language='JavaScript'>
						window.location = 'admin.php?page=messaging&action=reply&mid=<?php echo intval( $_POST[ 'mid' ] ); ?>';
					</script>
					<?php
				}
						...
						<tr valign="top">
							...
							<td>
								...
								<?php } else { ?>
									...
									?> <?php echo $rows; ?> style="width: 95%" name='message_content' tabindex='1' id='message_content'></textarea>
									<?php } ?>
		...
		case "reply_process":
			if ( $_POST[ 'message_to' ] == '' || $_POST[ 'message_subject' ] == '' || $_POST[ 'message_content' ] == '' ) {
					...
					<table class="form-table">
						...
						<tr valign="top">
							...
							<td><textarea <?php
								...
								?> <?php echo $rows; ?> style="width: 95%" name='message_content' tabindex='1' id='message_content'><?php echo wp_kses_post( $_POST[ 'message_content' ] ); ?></textarea>
								<br />
								<?php _e( 'Required', 'messaging' ) ?></td>
						</tr>
					</table>
				...
				if ( $tmp_username_error > 0 ) {
						...
						<table class="form-table">
							...
							<tr valign="top">
								...
								<td><textarea <?php
									...
									?> <?php echo $rows; ?> style="width: 95%" name='message_content' tabindex='3' id='message_content'><?php echo wp_kses_post( $_POST[ 'message_content' ] ); ?></textarea>
									<br />
									<?php _e( 'Required', 'messaging' ) ?></td>
							</tr>
						</table>
					...
					?>
					<script language='JavaScript'>
						window.location = 'admin.php?page=messaging&updated=true&updatedmsg=<?php echo urlencode( __( 'Reply Sent.', 'messaging' ) ); ?>';
					</script>
					<?php
				}
			}
						...
						<tr valign="top">
							...
							<td>
								...
								<?php } else { ?>
									...
									?> <?php echo $rows; ?> style="width: 95%" name='message_content' tabindex='1' id='message_content'><?php echo isset( $_POST[ 'message_content' ] ) ? wp_kses_post( $_POST[ 'message_content' ] ) : ''; ?></textarea>
									<?php } ?>
								<br />
								<?php _e( 'Required', 'messaging' ) ?></td>
						</tr>
		...
		case "process":
			if ( $_POST[ 'message_to' ] == '' || $_POST[ 'message_subject' ] == '' || $_POST[ 'message_content' ] == '' ) {
				...
				if ( $tmp_username_error > 0 ) {
					...
					?>
					<script language='JavaScript'>
						window.location = 'admin.php?page=messaging&amp;updated=true&amp;updatedmsg=<?php echo urlencode( __( 'Message(s) Sent.', 'messaging' ) ); ?>';
					</script>
					<?php
				}
			}
...
function messaging_notifications_page_output() {
	...
	switch ( $action ) {
					...
					<tr valign="top">
						...
						<td>
							<select name="message_email_notification" id="message_email_notification">
								...
								if ( $tmp_message_email_notification == 'yes' ) {
									echo 'selected="selected"';
								}
								?> ><?php _e( 'Yes', 'messaging' ) ?></option>
								<option value="no" <?php
								if ( $tmp_message_email_notification == 'no' ) {
									echo 'selected="selected"';
								}
								?> ><?php _e( 'No', 'messaging' ) ?></option>
							</select>
						</td>
					</tr>
				</table>
		...
		case "process":
			...
			?>
			<script language='JavaScript'>
				window.location = 'admin.php?page=messaging_message-notifications&updated=true&updatedmsg=<?php echo urlencode( __( 'Settings saved.', 'messaging' ) ); ?>';
			</script>
			<?php
			break;
		//---------------------------------------------------//
